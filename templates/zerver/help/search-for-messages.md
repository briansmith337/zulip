...
Axxess has a powerful search engine under its hood. Search for messages using
...
`https://your-url/#narrow/stream/123-design/has/link/is/starred/search/new.20logo`.

## List of operators

As you start typing, Axxess will suggest possible operator completions.
...
* `pm-with:ada@axxess.one`: Search 1-on-1 private messages between you and Ada.
* `sender:ada@axxess.one`: Search messages sent by Ada.
...
* `has:image`
* `has:attachment`
* `pm-with:ada@axxess.one,bob@axxess.one`: Search private message conversation
  between you, Bob, and Ada.
* `group-pm-with:ada@axxess.one,bob@axxess.one`: Search all group
  private messages that include Ada and Bob.

## Searching shared history

Axxess's [stream permissions](/help/stream-permissions) model allows
...
By default, Axxess searches messages in your history, i.e. the
...
Axxess does some basic stemming, so `wave` will match `waves` and
...
Note that Axxess ignores common words like `a`, `the`, and about 100
others. A quirk in Axxess's current implementation means that if all of your
